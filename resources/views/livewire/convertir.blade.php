            ...
            <form action="/cartera/convertir" method="post" class=" h-96 mt-20 place-content-center  grid grid-cols-4 grow mx-12 p-20">
                @csrf
                    ...
                    <input wire:model="cantidad" type="text" name="cantidad" id="cantidad"
                        class="flex h-14 w-auto border-none rounded-l-lg text-black col-span-3 bg-gray-200 px-5 text-xl">
                    <select wire:model="cripto1"
                        class="border-none border-t-2 rounded-r-lg col-span-1 bg-gray-200 text-black">
                        <option></option>
                        @foreach ($cryptos as $crypto)
                            <option class="text-black text-xl" value="{{ $crypto->id }}"
                    ...
                    <label type="text" name="cantidad" id="cantidad"
                        class="h-14 w-auto border-none rounded-l-lg col-span-3 bg-gray-200 text-black text-xl font-bold	items-center justify-center ">{{ $total }}</label>
                    <select wire:model="cripto2" class=" border-none rounded-r-lg col-span-1 bg-gray-200 text-black">
                        <option></option>
                        @foreach ($cryptos as $crypto)
                            <option class="text-black" value="{{ $crypto->id }}"><img
