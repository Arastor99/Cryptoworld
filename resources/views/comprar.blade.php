            ...
            <form action="/cartera/convertir" method="post" class=" h-96 mt-20 place-content-center  grid grid-cols-4 grow mx-12 p-20">
                @csrf
                    ...
                    <label type="text" name="cantidad" id="cantidad"
                        class="h-14 w-auto border-none rounded-l-lg col-span-3 bg-gray-200 text-black text-xl font-bold flex items-center ">{{ $total }}</label>
                    <select wire:model="cripto2" class=" border-none rounded-r-lg col-span-1 bg-gray-200 text-black text-center">
                        <option></option>
                        @foreach ($cryptos as $crypto)
